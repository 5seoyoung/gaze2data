...
class RoIGazeTrackerGaze360:
    def __init__(
        ...
        self.person_gaze_data: dict[int, dict] = defaultdict(
            lambda: {
                ...
                "last_gaze": None,
                "clothing_color_class": None,  # 옷 색상 정보
            }
        )
    ...
    def extract_clothing_color(self, frame, bbox):
        """사람 bounding box의 상체 영역에서 옷 색상 추출"""
        ...
        # 상체 영역 추출 (bbox의 상단 30%~60% 영역)
        body_height = y2 - y1
        upper_start = int(y1 + body_height * 0.3)
        upper_end = int(y1 + body_height * 0.6)
        
        upper_start = max(y1, min(upper_start, h))
        upper_end = max(upper_start, min(upper_end, h))
        
        if upper_end <= upper_start:
            return None
        
        # 상체 영역 ROI 추출
        upper_body_roi = frame[upper_start:upper_end, x1:x2]
        
        if upper_body_roi.size == 0:
            return None
        
        # 평균 색상 계산 (BGR)
        mean_color = np.mean(upper_body_roi.reshape(-1, 3), axis=0)
        return mean_color.astype(int).tolist()
    
    def classify_clothing_color(self, color_bgr):
        """옷 색상을 검은색/흰색/회색으로 분류"""
        ...
        if brightness < 60:
            ...
            return "white"
        elif saturation < 50:  # 채도가 낮으면 회색 계열
            return "gray"
        else:
            # 채도가 높으면 회색으로 분류 (다른 색상은 무시)
            return "gray"
    
    def simple_track(self, detections, frame_count: int, frame=None):
        """간단한 IoU 기반 추적 + 옷 색상 기반 추적"""
        global next_id, tracked_objects, data_deque
        ...
        if frame is not None:
            for i, det in enumerate(detections):
                ...
                color_bgr = self.extract_clothing_color(frame, [x1, y1, x2, y2])
                color_class = self.classify_clothing_color(color_bgr)
                detection_colors[i] = {
                    "color_bgr": color_bgr,
        ...
        for i, det in enumerate(detections):
            ...
            # 현재 탐지의 옷 색상
            det_color_info = detection_colors.get(i, {})
            det_color_class = det_color_info.get("color_class")
            ...
            for obj_id, obj_data in tracked_objects.items():
                ...
                # IoU 매칭
                if iou > best_iou:
                    # 옷 색상도 확인 (같은 색상이면 우선순위 높임)
                    obj_color_class = obj_data.get("clothing_color_class")
                    if det_color_class and obj_color_class:
                        if det_color_class == obj_color_class:
                            # 같은 색상이면 IoU 임계값을 낮춰서 매칭
                            if iou > max(0.2, best_iou * 0.7):
                                best_iou = iou
                                best_match = obj_id
                        else:
                            # 다른 색상이면 IoU 임계값을 높여서 매칭 방지
                            if iou > 0.5:
                                best_iou = iou
                                best_match = obj_id
                    else:
            ...
            if best_match is not None:
                ...
                # 옷 색상 정보 업데이트
                if det_color_info:
                    tracked_objects[best_match]["clothing_color_bgr"] = det_color_info.get("color_bgr")
                    tracked_objects[best_match]["clothing_color_class"] = det_color_info.get("color_class")
                ...
                new_obj = {
                    ...
                    "gaze_history": deque(maxlen=max(1, self.smoothing_window)) if self.smoothing_window > 0 else None,
                }
                # 옷 색상 정보 추가
                if det_color_info:
                    new_obj["clothing_color_bgr"] = det_color_info.get("color_bgr")
                    new_obj["clothing_color_class"] = det_color_info.get("color_class")
                
                tracked_objects[next_id] = new_obj
                next_id += 1
                matched_detections.add(i)
    ...
    def draw_boxes_with_gaze(
        ...
        for i, box in enumerate(bbox):
            ...
            # 옷 색상 정보 추가
            clothing_color_class = None
            if track_id in tracked_objects:
                clothing_color_class = tracked_objects[track_id].get("clothing_color_class")
            
            if clothing_color_class:
                color_map = {"black": "검정", "white": "흰색", "gray": "회색"}
                color_kr = color_map.get(clothing_color_class, clothing_color_class)
                label = f"ID:{track_id} {obj_name} [{color_kr}]"
            else:
                ...
                if person_roi.size > 0:
                    ...
                    if (
                        ...
                        if final_result is not None:
                            ...
                            current_roi, _ = self.select_roi_by_min_distance(
                                face_center_x,
                                face_center_y,
                                mapped_gaze_vector,
                            )
                            ...
                            # 옷 색상 정보 저장
                            if track_id in tracked_objects:
                                clothing_color = tracked_objects[track_id].get("clothing_color_class")
                                if clothing_color:
                                    self.person_gaze_data[track_id]["clothing_color_class"] = clothing_color
                            ...
                            # 시선 반직선(2D 프로젝션) 시각화: Gaze360과 동일 부호(음수) 사용
                            if self.show_gaze_ray:
                                ...
                                if dir_norm > 1e-6:
                                    gaze_dir /= dir_norm
                                    h, w = img.shape[:2]
                                    ray_len = float(max(h, w) * 2.0)
                                    ray_end = (
                                        int(face_center_x + gaze_dir[0] * ray_len),
                                        int(face_center_y + gaze_dir[1] * ray_len),
                                    )
                                    cv2.line(
                                        img,
    ...
    def process_frame(self, frame, frame_count: int = 0):
        ...
        if len(detections) > 0:
            tracks = self.simple_track(detections, frame_count, frame=frame)
            if len(tracks) > 0:
                bbox_xyxy = []
                identities = []
                object_id = []
        ...
        try:
            ...
            for person_id, person_data in self.person_gaze_data.items():
                if len(person_data["gaze_journey"]) > 0:
                    ...
                    # 결과 출력 형식: ROI 번호만 (예: 1,2,7)
                    roi_str = ",".join(map(str, sorted(set(roi_numbers))))
                    data.append(
                        {
                            "person_id": person_id,
                            "roi_numbers": roi_str,
            ...
            for person_id, person_data in sorted(
                ...
                if len(person_data["gaze_journey"]) > 0:
                    # ROI 번호만 추출하여 출력
                    roi_numbers = sorted(set(roi_id for roi_id, _ in person_data["gaze_journey"]))
                    roi_str = ",".join(map(str, roi_numbers))
